...
{
	uint32_t regvalue = 0, linkStatus = 0;
	struct netif *netif = (struct netif *)argument;
	...
	if(((regvalue & PHY_LINKED_STATUS)!= (uint16_t)RESET) && (linkStatus == 0))
	{
			netif_set_link_up(netif);
			linkStatus = 1;
	}
	else if (((regvalue & PHY_LINKED_STATUS)== (uint16_t)RESET) && (linkStatus == 1))
	{
			netif_set_link_down(netif);
			linkStatus = 0;
	}
		osDelay(INTERFACE_LINK_STATUS_PERIOD);
	}
}
